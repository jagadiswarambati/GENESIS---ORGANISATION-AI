...
class ExecutionEngineService:
    ...
    async def execute_ready_tasks(self, request: ExecutionRequest) -> ExecutionResult:
        ...
        work_items: list[tuple[Task, AIWorker]] = []
        for task_id in ready_task_ids:
            task = tasks_by_id.get(task_id)
            worker = worker_by_task_id.get(task_id)
            if task is None or worker is None:
                continue
            work_items.append((task, worker))

        supports_parallel_execution = bool(
            getattr(self._provider, "supports_parallel_execution", True)
        )
        if supports_parallel_execution and len(work_items) > 1:
            executions = list(
                await asyncio.gather(
                    *(
                        self._provider.execute_task(task, worker, organization_memory)
                        for task, worker in work_items
                    )
                )
            )
        else:
            executions = [
                await self._provider.execute_task(task, worker, organization_memory)
                for task, worker in work_items
            ]

        for (task, worker), execution in zip(work_items, executions, strict=True):
